...
const ticketsController = {
  ...
  createTicket: async (req, res) => {
    try {
      ...
      console.log("agentTicketCount1", agentTicketCount);
      // Check if the agent has less than 5 tickets
          ...
          case 'low':
            switch (Category) {
              ...
              case 'hardware':
                ...
                agentTicketCount = (result && result.softwareCount);
                console.log("Resultt:", agentTicketCount);
                break;
              case 'network':
                result = await TicketsModel.findOneAndUpdate(
                  { Status: 'open' },
                ...
                agentTicketCount = (result && result.hardwareCount);
                break;
              default:
                return res.status(400).json({ message: "Must enter a valid category" });
            }
          ...
          case 'medium':
            switch (Category) {
              ...
              case 'hardware':
                ...
                agentTicketCount = (result && result.networkCount);
                console.log("Resultttt:", agentTicketCount);
                break;
              case 'network':
                result = await TicketsModel.findOneAndUpdate(
                  { Status: 'open' },
